...
static int Split_eatAn(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	while(isalnum(*p)) {
		if(isdigit(*p)) {
			...
			dif =(int) QRinput_estimateBitsModeAn((int)p - (int)string) /* + 4 + la */
				+(int) QRinput_estimateBitsModeNum((int)q -(int) p) + 4 + ln
				-(int) QRinput_estimateBitsModeAn((int)q - (int)string) /* - 4 - la */;
			if(dif < 0) {
				break;
			} else {
				p = q;
